...
import { reopenClass } from "@/services/server/classCouncilService";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ councilId: string; classId: string }> }) {
  try {
    ...
    return NextResponse.json(await reopenClass(parseUuid(councilId, "Conselho"), parseUuid(classId, "Turma"), user.id));
  } catch (error) {
