  ...
  return <TooltipProvider delayDuration={250}><main className="mx-auto w-full max-w-7xl p-4 py-8 sm:p-6 lg:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px]">
    ...
    <header className="rounded-3xl border bg-white p-5 shadow-sm dark:bg-slate-900 sm:p-7">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4"><div className="grid size-14 shrink-0 place-items-center rounded-2xl bg-slate-950 text-lg font-black text-white dark:bg-white dark:text-slate-950">{initials(data.student.name)}</div><div className="min-w-0"><p className="text-sm font-bold uppercase tracking-[0.16em] text-sky-700 dark:text-sky-300">Perfil do estudante</p><h1 className="mt-1 truncate text-2xl font-black tracking-tight sm:text-3xl">{data.student.name}</h1><p className="mt-1 text-sm text-muted-foreground">Matrícula {data.student.enrollmentNumber}{latest ? ` · Turma ${latest.class.name}` : ""}</p></div></div>
    ...
    {!latest ? <><section className="mt-6 rounded-3xl border border-dashed bg-white p-12 text-center dark:bg-slate-900"><UserRound className="mx-auto size-9 text-slate-400" /><h2 className="mt-4 text-lg font-bold">Sem histórico confirmado</h2><p className="mt-2 text-sm text-muted-foreground">O estudante está cadastrado, mas ainda não aparece em uma versão ativa de relatório do Conselho de Classe.</p></section><InterventionSection studentId={data.student.id} interventions={data.interventions} /><OccurrenceSection studentId={data.student.id} occurrences={data.occurrences} /></> : <>
      ...
      <section className="mt-6 rounded-3xl border bg-white p-5 shadow-sm dark:bg-slate-900 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-sky-700 dark:text-sky-300">Situação mais recente</p><h2 className="mt-2 text-xl font-black">{latest.council.term}º bimestre de {latest.council.schoolYear}</h2><p className="mt-1 text-sm text-muted-foreground">{latest.class.name} · {latest.class.gradeLevel ? `${latest.class.gradeLevel}ª série` : "Série não identificada"} · {shiftLabels[latest.class.shift] ?? latest.class.shift} · Conselho de {formatDate(latest.council.meetingDate)}</p></div><span className={`rounded-full border px-3 py-1.5 text-sm font-bold ${academicTone(latest.alerts.academicStatus)}`}>{academicLabels[latest.alerts.academicStatus]}</span></div>
...
function StudentSituationControl({ studentId, currentSituation, situationUpdatedAt }: { studentId: string; currentSituation: AttendanceSituation; situationUpdatedAt: string | null }) {
  ...
  return <><button type="button" onClick={() => { setSituation(currentSituation); setOpen(true); }} className={`group flex items-center gap-3 rounded-xl border px-3.5 py-2 text-left transition hover:brightness-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${situationTone(currentSituation)}`}><span><span className="block text-sm font-bold uppercase tracking-[0.12em] opacity-70">Situação atual</span><strong className="block text-sm">{STUDENT_SITUATION_LABELS[currentSituation]}</strong></span><PencilLine className="size-3.5 opacity-60 transition group-hover:opacity-100" /></button>
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!saving) setOpen(nextOpen); }}><DialogContent className="rounded-2xl sm:max-w-lg"><DialogHeader><DialogTitle>Frequência e vínculo</DialogTitle><DialogDescription>Esta é a situação permanente do estudante e pode ser alterada mesmo quando ele não aparece no relatório mais recente.</DialogDescription></DialogHeader><div className="space-y-4 py-4"><div className="space-y-2"><Label>Situação atual</Label><Select value={situation} onValueChange={(value) => setSituation(value as AttendanceSituation)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{STUDENT_SITUATIONS.map((item) => <SelectItem key={item} value={item}>{STUDENT_SITUATION_LABELS[item]}</SelectItem>)}</SelectContent></Select></div><div className={`rounded-xl border p-3 text-sm leading-5 ${situationTone(situation)}`}>{STUDENT_SITUATION_DESCRIPTIONS[situation]}</div>{situationUpdatedAt ? <p className="text-sm text-muted-foreground">Última alteração em {new Date(situationUpdatedAt).toLocaleString("pt-BR")}.</p> : null}</div><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={saving}>Cancelar</Button><Button type="button" onClick={() => void save()} disabled={saving || situation === currentSituation}>{saving ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{saving ? "Salvando..." : "Salvar situação"}</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
...
function InterventionSection({ studentId, interventions }: { studentId: string; interventions: StudentProfileData["interventions"] }) {
  ...
  async function updateStatus(id: string, status: InterventionStatus) {
    ...
    try {
      ...
      await Promise.all([queryClient.invalidateQueries({ queryKey: ["student-profile", studentId] }), queryClient.invalidateQueries({ queryKey: ["interventions"] }), queryClient.invalidateQueries({ queryKey: ["pedagogical-dashboard"] })]);
  ...
  function originLabel(item: StudentProfileData["interventions"][number]) {
    if (item.sourceType === "class_council") return `Conselho · ${item.origin.term}º bimestre de ${item.origin.schoolYear}${item.origin.className ? ` · Turma ${item.origin.className}` : ""}`;
    if (item.sourceType === "student_profile") return `Criada no prontuário${item.targetClass?.className ? ` · Turma ${item.targetClass.className}` : ""}`;
    return `Criada na Central de intervenções${item.targetClass?.className ? ` · Turma ${item.targetClass.className}` : ""}`;
  }

  return <section className="mt-7 rounded-3xl border bg-white p-5 shadow-sm dark:bg-slate-900 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-sky-700 dark:text-sky-300">Acompanhamento pedagógico</p><h2 className="mt-2 text-xl font-black">Intervenções</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">Ações pedagógicas acompanhadas até a execução ou o cancelamento, criadas no Conselho, no prontuário ou na Central.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" className="rounded-xl" onClick={() => setCreating(true)}><Plus className="size-4" />Nova intervenção</Button><Button asChild variant="outline" className="rounded-xl"><Link href="/hub/intervencoes"><ClipboardCheck className="size-4" />Central de intervenções</Link></Button></div></div>
    {ordered.length ? <div className="mt-5 space-y-3">{ordered.map((item) => { const overdue = (item.status === "pending" || item.status === "in_progress") && Boolean(item.dueDate && item.dueDate < new Intl.DateTimeFormat("en-CA", { timeZone: "America/Maceio", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date())); return <article key={item.id} className="rounded-2xl border p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-sm font-semibold leading-6">{item.description}</p><p className="mt-1 text-sm text-muted-foreground">{originLabel(item)}</p>{item.reason ? <p className="mt-2 text-sm leading-5 text-muted-foreground"><strong className="text-foreground">Motivo:</strong> {item.reason}</p> : null}</div><Select disabled={updatingId === item.id} value={item.status} onValueChange={(value) => void updateStatus(item.id, value as InterventionStatus)}><SelectTrigger className={`h-9 w-[146px] text-sm font-bold ${item.status === "completed" ? "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/35 dark:text-emerald-200" : item.status === "cancelled" ? "border-slate-200 bg-slate-100 text-slate-700 dark:border-slate-700 dark:bg-slate-800" : item.status === "in_progress" ? "border-sky-200 bg-sky-50 text-sky-800 dark:border-sky-900 dark:bg-sky-950/35 dark:text-sky-200" : "border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/35 dark:text-amber-200"}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pending">Pendente</SelectItem><SelectItem value="in_progress">Em andamento</SelectItem><SelectItem value="completed">Concluída</SelectItem><SelectItem value="cancelled">Cancelada</SelectItem></SelectContent></Select></div><div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted-foreground"><span>Responsável: <strong className="text-foreground">{item.responsibleName ?? "Não definido"}</strong></span><span className={overdue ? "font-bold text-rose-700 dark:text-rose-300" : ""}>Prazo: <strong>{formatDate(item.dueDate)}</strong>{overdue ? " · atrasada" : ""}</span></div>{item.outcome ? <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm leading-5 text-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200"><strong>Resultado:</strong> {item.outcome}</p> : null}{item.cancellationReason ? <p className="mt-3 rounded-xl bg-slate-100 p-3 text-sm leading-5 text-slate-700 dark:bg-slate-800 dark:text-slate-200"><strong>Cancelamento:</strong> {item.cancellationReason}</p> : null}</article>; })}</div> : <div className="mt-5 rounded-2xl border border-dashed p-8 text-center"><ClipboardCheck className="mx-auto size-7 text-slate-400" /><p className="mt-3 text-sm font-semibold">Nenhuma intervenção registrada</p><p className="mt-1 text-sm text-muted-foreground">Crie uma ação para iniciar o acompanhamento deste estudante.</p><Button variant="outline" className="mt-4 rounded-xl" onClick={() => setCreating(true)}><Plus className="size-4" />Criar primeira intervenção</Button></div>}
    <CreateStudentInterventionDialog studentId={studentId} sourceType="student_profile" open={creating} onOpenChange={setCreating} onCreated={async () => { await Promise.all([queryClient.invalidateQueries({ queryKey: ["student-profile", studentId] }), queryClient.invalidateQueries({ queryKey: ["interventions"] }), queryClient.invalidateQueries({ queryKey: ["pedagogical-dashboard"] })]); }} />
  </section>;
}

function CreateStudentInterventionDialog({ studentId, studentName, sourceType, open, onOpenChange, onCreated }: { studentId: string; studentName?: string; sourceType: "student_profile" | "intervention_center"; open: boolean; onOpenChange: (open: boolean) => void; onCreated: () => Promise<void> | void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await councilFetch("/api/interventions", { method: "POST", body: JSON.stringify({ sourceType, studentId, description, reason, responsibleName, dueDate }) });
      await onCreated();
      setDescription(""); setReason(""); setResponsibleName(""); setDueDate(""); onOpenChange(false);
      toast.success("Intervenção criada e adicionada ao acompanhamento.");
    } catch (error) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!saving) onOpenChange(next); }}><DialogContent className="rounded-2xl sm:max-w-xl"><form onSubmit={submit}><DialogHeader><DialogTitle>Nova intervenção</DialogTitle><DialogDescription>{studentName ? `Registre uma ação para ${studentName}.` : "Registre a ação e o contexto que justificou o acompanhamento."}</DialogDescription></DialogHeader><div className="grid gap-4 py-5 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label htmlFor="intervention-action">Ação/intervenção</Label><Textarea id="intervention-action" value={description} onChange={(event) => setDescription(event.target.value)} rows={3} maxLength={2000} placeholder="Ex.: conversar com o estudante e acompanhar a entrega das atividades" required /></div><div className="space-y-2 sm:col-span-2"><Label htmlFor="intervention-reason">Motivo ou contexto</Label><Textarea id="intervention-reason" value={reason} onChange={(event) => setReason(event.target.value)} rows={3} maxLength={2000} placeholder="Descreva objetivamente o que motivou esta intervenção" required /></div><div className="space-y-2"><Label htmlFor="intervention-responsible">Responsável (opcional)</Label><Input id="intervention-responsible" value={responsibleName} onChange={(event) => setResponsibleName(event.target.value)} maxLength={200} /></div><div className="space-y-2"><Label htmlFor="intervention-due-date">Prazo (opcional)</Label><Input id="intervention-due-date" type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></div></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={saving}>Cancelar</Button><Button type="submit" disabled={saving || !description.trim() || !reason.trim()}>{saving ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{saving ? "Criando..." : "Criar intervenção"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
function OccurrenceSection({ studentId, occurrences }: { studentId: string; occurrences: StudentProfileData["occurrences"] }) {
  ...
  return <section className="mt-7 rounded-3xl border bg-white p-5 shadow-sm dark:bg-slate-900 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-amber-700 dark:text-amber-300">Prontuário disciplinar</p><h2 className="mt-2 text-xl font-black">Ocorrências</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">Registros factuais da vida escolar. Eles não alteram automaticamente o risco acadêmico nem os indicadores de comportamento do conselho.</p></div><Button className="rounded-xl" onClick={() => setOpen(true)}><Plus className="size-4" />Registrar ocorrência</Button></div>
    {occurrences.length ? <div className="mt-5 divide-y rounded-2xl border">{occurrences.map((occurrence) => <article key={occurrence.id} className="p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{STUDENT_OCCURRENCE_LABELS[occurrence.category]}</strong>{occurrence.targetType === "collective" ? <span className="rounded-full bg-amber-100 px-2 py-0.5 text-sm font-bold uppercase text-amber-800 dark:bg-amber-950/60 dark:text-amber-200">Coletiva · {occurrence.participants.length} estudantes</span> : null}</div><p className="mt-1 text-sm text-muted-foreground">{formatDate(occurrence.occurredOn)}{occurrence.createdByName ? ` · Registrado por ${occurrence.createdByName}` : ""}</p></div><span className={`rounded-full px-2.5 py-1 text-sm font-bold ${occurrence.guardianNotified ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}>{occurrence.guardianNotified ? "Responsável ciente" : "Responsável não cientificado"}</span></div>{occurrence.targetType === "collective" ? <p className="mt-3 text-sm leading-5 text-muted-foreground"><strong className="text-foreground">Estudantes envolvidos:</strong> {occurrence.participants.map((item) => `${item.name}${item.className ? ` (${item.className})` : ""}`).join(", ")}</p> : null}{occurrence.notes ? <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700 dark:text-slate-300">{occurrence.notes}</p> : null}</article>)}</div> : <div className="mt-5 rounded-2xl border border-dashed p-8 text-center"><TriangleAlert className="mx-auto size-7 text-slate-400" /><p className="mt-3 text-sm font-semibold">Nenhuma ocorrência registrada</p><p className="mt-1 text-sm text-muted-foreground">O histórico será exibido aqui em ordem cronológica.</p></div>}

    <Dialog open={open} onOpenChange={(nextOpen) => { if (!saving) setOpen(nextOpen); }}><DialogContent className="rounded-2xl sm:max-w-xl"><form onSubmit={submit}><DialogHeader><DialogTitle>Registrar ocorrência</DialogTitle><DialogDescription>Descreva o fato de maneira objetiva. O registro ficará vinculado permanentemente ao estudante.</DialogDescription></DialogHeader><div className="grid gap-4 py-5 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="occurrence-date">Data</Label><Input id="occurrence-date" type="date" value={occurredOn} onChange={(event) => setOccurredOn(event.target.value)} max={new Intl.DateTimeFormat("en-CA", { timeZone: "America/Maceio", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date())} required /></div><div className="space-y-2"><Label>Responsável ciente</Label><Select value={guardianNotified} onValueChange={setGuardianNotified}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="yes">Sim</SelectItem><SelectItem value="no">Não</SelectItem></SelectContent></Select></div><div className="space-y-2 sm:col-span-2"><Label>Tipo de ocorrência</Label><Select value={category} onValueChange={(value) => setCategory(value as StudentOccurrenceCategory)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{STUDENT_OCCURRENCE_CATEGORIES.map((item) => <SelectItem key={item} value={item}>{STUDENT_OCCURRENCE_LABELS[item]}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2 sm:col-span-2"><Label htmlFor="occurrence-notes">Observações {category === "other" ? "" : "(opcional)"}</Label><Textarea id="occurrence-notes" value={notes} onChange={(event) => setNotes(event.target.value)} maxLength={2000} rows={5} placeholder="Contexto objetivo do ocorrido, providências tomadas e informações relevantes." required={category === "other"} /><p className="text-right text-sm text-muted-foreground">{notes.length}/2000</p></div></div><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={saving}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{saving ? "Registrando..." : "Registrar ocorrência"}</Button></DialogFooter></form></DialogContent></Dialog>
  </section>;
}

function SummaryCard({ icon: Icon, value, label }: { icon: typeof CalendarDays; value: string | number; label: string }) {
  return <div className="rounded-2xl border bg-white p-4 shadow-sm dark:bg-slate-900"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"><Icon className="size-4" /></span><div><strong className="block text-xl font-black">{typeof value === "number" ? value.toLocaleString("pt-BR") : value}</strong><span className="text-sm text-muted-foreground">{label}</span></div></div></div>;
}
...
function StatusItem({ icon: Icon, label, value, detail }: { icon: typeof GraduationCap; label: string; value: string; detail: string }) {
  return <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800/70"><div className="flex items-center gap-2 text-sm font-bold text-muted-foreground"><Icon className="size-3.5" />{label}</div><strong className="mt-2 block text-sm">{value}</strong><p className="mt-1 text-sm leading-5 text-muted-foreground">{detail}</p></div>;
}
...
function SubjectTable({ item }: { item: StudentCouncilHistoryItem }) {
  ...
  return <div><div className="mb-3 flex items-end justify-between gap-3"><div><h3 className="font-extrabold">Desempenho por componente</h3><p className="mt-1 text-sm text-muted-foreground">Notas e faltas registradas até o {item.council.term}º bimestre.</p></div><span className="text-sm text-muted-foreground">{subjects.length} componentes</span></div>
    {subjects.length ? <div className="overflow-x-auto rounded-2xl border"><table className="w-full min-w-[620px] text-sm"><thead className="bg-slate-50 dark:bg-slate-800"><tr><th className="px-4 py-3 text-left text-sm font-bold text-muted-foreground">Componente</th>{Array.from({ length: item.council.term }, (_, index) => <th key={index} className="w-24 px-3 py-3 text-center text-sm font-bold text-muted-foreground">{index + 1}º bim.</th>)}<th className="w-24 px-3 py-3 text-center text-sm font-bold text-muted-foreground">Pontos</th></tr></thead><tbody>{subjects.map((subject) => { const results = item.results.filter((result) => result.subjectId === subject.id); const total = results.reduce((sum, result) => sum + (result.grade ?? 0), 0); return <tr key={subject.id} className="border-t"><td className="px-4 py-3"><div className="flex items-center gap-2"><Tooltip><TooltipTrigger asChild><span tabIndex={0} className="min-w-9 cursor-help rounded-md bg-slate-100 px-1.5 py-1 text-center text-sm font-black dark:bg-slate-800">{subjectAbbreviation(subject.name)}</span></TooltipTrigger><TooltipContent>{subject.name}</TooltipContent></Tooltip><div><strong className="block text-sm">{subject.name}</strong>{subject.teacher ? <span className="text-sm text-muted-foreground">{subject.teacher}</span> : null}</div></div></td>{Array.from({ length: item.council.term }, (_, index) => { const result = results.find((entry) => entry.term === index + 1); return <td key={index} className="px-3 py-3 text-center"><strong className="text-sm">{result?.grade !== null && result?.grade !== undefined ? result.grade.toLocaleString("pt-BR") : result?.gradeMarker || "—"}</strong>{result?.absences !== null && result?.absences !== undefined ? <span className="mt-0.5 block text-sm text-muted-foreground">{result.absences} {result.absences === 1 ? "falta" : "faltas"}</span> : null}</td>; })}<td className="px-3 py-3 text-center font-black">{total.toLocaleString("pt-BR")}</td></tr>; })}</tbody></table></div> : <div className="rounded-2xl border border-dashed p-6 text-center text-sm text-muted-foreground">Nenhum resultado disponível nesta versão.</div>}
  </div>;
}

function CouncilRecords({ item }: { item: StudentCouncilHistoryItem }) {
  return <aside><h3 className="font-extrabold">Registros do conselho</h3><p className="mt-1 text-sm text-muted-foreground">Informações registradas pela equipe.</p><div className="mt-3 space-y-3">
    ...
    <div className="rounded-2xl border p-4"><div className="flex items-center justify-between gap-3"><strong className="text-sm">Comportamento</strong><span className="text-sm text-muted-foreground">{item.behaviors.length}</span></div>{item.behaviors.length ? <ul className="mt-3 space-y-2">{item.behaviors.map((behavior) => <li key={behavior.id} className="text-sm leading-5"><span className="font-semibold">{BEHAVIOR_LABELS[behavior.category] ?? behavior.category}</span>{behavior.description ? ` — ${behavior.description}` : ""}</li>)}</ul> : <p className="mt-2 text-sm text-muted-foreground">Nenhum registro de comportamento.</p>}</div>
  </div></aside>;
}

function RecordBlock({ title, content, empty }: { title: string; content: string | null; empty?: string }) {
  return <div className="rounded-2xl border p-4"><strong className="text-sm">{title}</strong><p className={`mt-2 text-sm leading-5 ${content ? "text-foreground" : "text-muted-foreground"}`}>{content || empty || "Não informado."}</p></div>;
}
