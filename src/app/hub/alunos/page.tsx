...
export default function StudentsDirectoryPage() {
  ...
  return <main className="mx-auto w-full max-w-7xl p-4 py-8 sm:p-6 lg:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px]">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      <Metric icon={UsersRound} value={data.metrics.total} label="cadastrados" active={situation === "all"} onClick={() => setSituation("all")} />
      <Metric icon={GraduationCap} value={data.metrics.current} label="no relatório atual" active={situation === "current"} onClick={() => setSituation((current) => current === "current" ? "all" : "current")} />
      <Metric icon={Footprints} value={data.metrics.infrequent} label="infrequentes" active={situation === "infrequent"} onClick={() => setSituation((current) => current === "infrequent" ? "all" : "infrequent")} />
      <Metric icon={UserMinus} value={data.metrics.dropout} label="desistentes" active={situation === "dropout"} onClick={() => setSituation((current) => current === "dropout" ? "all" : "dropout")} />
      <Metric icon={ArrowLeftRight} value={data.metrics.transferred} label="transferidos" active={situation === "transferred"} onClick={() => setSituation((current) => current === "transferred" ? "all" : "transferred")} />
      <Metric icon={TriangleAlert} value={data.metrics.occurrences} label="ocorrências" active={situation === "with_occurrences"} onClick={() => setSituation((current) => current === "with_occurrences" ? "all" : "with_occurrences")} />
    </section>
...
function Metric({ icon: Icon, value, label, active, onClick }: { icon: typeof UsersRound; value: number; label: string; active: boolean; onClick: () => void }) {
  return <button type="button" aria-pressed={active} onClick={onClick} className={`group w-full rounded-2xl border bg-white p-4 text-left shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 dark:bg-slate-900 dark:focus-visible:ring-offset-slate-950 ${active ? "border-sky-300 ring-1 ring-sky-200 dark:border-sky-700 dark:ring-sky-900" : "hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md dark:hover:border-sky-800"}`}><div className="flex items-center gap-3"><span className={`grid size-9 shrink-0 place-items-center rounded-xl transition ${active ? "bg-sky-100 text-sky-700 dark:bg-sky-950 dark:text-sky-300" : "bg-slate-100 text-slate-700 group-hover:bg-sky-50 group-hover:text-sky-700 dark:bg-slate-800 dark:text-slate-200 dark:group-hover:bg-sky-950/60 dark:group-hover:text-sky-300"}`}><Icon className="size-4" /></span><div className="min-w-0"><strong className="block text-xl font-black">{value.toLocaleString("pt-BR")}</strong><span className={`text-[11px] transition ${active ? "font-semibold text-sky-700 dark:text-sky-300" : "text-muted-foreground"}`}>{label}</span></div></div></button>;
}
