...
export default function PedagogicalDashboardPage() {
  ...
  return <TooltipProvider delayDuration={350}><main className="mx-auto w-full max-w-7xl p-4 py-8 sm:p-6 lg:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px]">
    ...
    <section className="mb-5 flex flex-wrap items-center gap-x-6 gap-y-2 rounded-2xl border bg-white px-4 py-3 text-xs text-muted-foreground shadow-sm dark:bg-slate-900">
...
function SmallMetric({ value, label, description }: { value: number; label: string; description: string }) { return <Tooltip><TooltipTrigger asChild><span tabIndex={0} className="rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring"><strong className="block text-sm">{value}</strong><span className="text-[10px] text-muted-foreground underline decoration-dotted underline-offset-2">{label}</span></span></TooltipTrigger><TooltipContent side="top" className="max-w-64 text-xs leading-5">{description}</TooltipContent></Tooltip>; }
function QualityTotal({ value, label }: { value: number; label: string }) { return <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800"><strong className="text-xl font-black">{value}</strong><p className="mt-1 text-[11px] text-muted-foreground">{label}</p></div>; }

function SubjectAbbreviationList({ label, subjects }: { label: string; subjects: DashboardStudent["alerts"]["subjectDetails"] }) {
  if (!subjects.length) return null;
  return <p className="text-[11px] leading-5 text-muted-foreground"><span className="font-medium text-foreground">{label}:</span>{" "}{subjects.map((subject, index) => <span key={subject.subjectId}>{index > 0 ? ", " : ""}<Tooltip><TooltipTrigger asChild><span tabIndex={0} className="cursor-help font-semibold text-foreground underline decoration-dotted underline-offset-2">{subjectAbbreviation(subject.subjectName)}</span></TooltipTrigger><TooltipContent side="top">{subject.subjectName}</TooltipContent></Tooltip></span>)}</p>;
}
...
function StudentOccurrenceContext({ student }: { student: DashboardStudent }) {
  ...
  return <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50/70 px-2.5 py-2 text-[10px] text-amber-900 dark:border-amber-900 dark:bg-amber-950/25 dark:text-amber-200"><strong>{student.occurrences.count} {student.occurrences.count === 1 ? "ocorrência" : "ocorrências"}</strong><span className="mx-1.5">·</span>Última: {STUDENT_OCCURRENCE_LABELS[student.occurrences.latest.category]}</div>;
}
