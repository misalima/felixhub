...
function StudentSituationNote({ situation }: { situation: AttendanceSituation }) {
  ...
  return <p className={`mt-1 text-sm font-semibold ${tone}`}>{STUDENT_SITUATION_LABELS[situation]}{situation === "transferred" ? " · fora do fluxo" : ""}</p>;
}
...
export default function CouncilDashboardPage() {
  ...
  async function deleteImport(importId: string) {
    ...
    setError("");
    try {
      await councilFetch(`/api/class-councils/${councilId}/imports/${importId}`, { method: "DELETE" });
      queryClient.setQueryData<Overview>(classCouncilQueryKeys.council(councilId), (current) => current ? {
        ...current,
        imports: current.imports.filter((item) => item.id !== importId),
      } : current);
      await queryClient.invalidateQueries({ queryKey: classCouncilQueryKeys.council(councilId) });
      toast.success("Versão do relatório excluída.");
    } catch (err) {
      const message = err instanceof Error ? err.message : "Falha ao excluir a versão do relatório.";
      setError(message);
      toast.error(message);
    } finally {
  ...
  const { council } = data;
  const latestImportId = data.imports.length > 0
    ? data.imports.reduce((latest, item) => item.version > latest.version ? item : latest).id
    : null;
  ...
  return <main className="mx-auto max-w-7xl p-4 py-8 sm:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px]">
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-6">
      {metricCards.map(({ key, label, value, icon: Icon }) => <button key={key} type="button" onClick={() => { setActiveMetric(key); setDetailSearch(""); }} className={`group rounded-2xl border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${metricStyles[key].card}`}>
        ...
        <p className="mt-3 text-2xl font-bold">{value}</p><p className="text-sm text-muted-foreground">{label}</p><span className="mt-2 block text-sm font-medium text-muted-foreground/80">Ver detalhes</span>
      </button>)}
    </section>

    <div className="mt-6 grid gap-6 lg:grid-cols-[1.4fr_1fr]"><section className="rounded-2xl border bg-white p-5 dark:bg-card"><div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Turmas</h2><span className="text-sm text-muted-foreground">{completed}/{data.classes.length} concluídas</span></div>{data.classes.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">Importe o relatório para criar as turmas.</div> : <div className="space-y-2">{data.classes.map((item) => <div key={item.id} className="group flex items-center gap-2 rounded-xl border p-2 transition hover:border-primary/50 hover:bg-muted/20"><Link href={`/hub/conselhos/${councilId}/turmas/${item.id}`} className="flex min-w-0 flex-1 items-center justify-between gap-4 rounded-lg p-2"><div className="min-w-0"><strong>{item.display_name}</strong><span className="ml-2 text-sm text-muted-foreground">{item.official_code}</span><p className="mt-1 text-sm text-muted-foreground">{item.studentCount ?? 0} estudantes · {item.atRiskCount ?? 0} em risco</p></div><Badge variant="secondary" className={classStatusBadgeClass(item.status)}>{classStatusLabel(item.status)}</Badge></Link>{council.status === "completed" && <Button variant="ghost" size="icon" asChild className="shrink-0 text-muted-foreground hover:text-foreground"><a href={`/api/class-councils/${councilId}/classes/${item.id}/pdf`} aria-label={`Baixar PDF da turma ${item.display_name}`} title={`Baixar PDF da turma ${item.display_name}`}><Download className="h-4 w-4" /></a></Button>}</div>)}</div>}</section><section className="rounded-2xl border bg-white p-5 dark:bg-card"><h2 className="font-semibold">Disciplinas com mais notas baixas</h2><div className="mt-4 space-y-4">{data.subjectRanking.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados acadêmicos confirmados.</p> : data.subjectRanking.map((item) => <div key={item.name}><div className="mb-1 flex justify-between gap-3 text-sm"><span className="truncate">{item.name}</span><strong>{item.low} · {item.percentage}%</strong></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-amber-500" style={{ width: `${item.percentage}%` }} /></div></div>)}</div></section></div>

    {data.imports.length > 0 && <section className="mt-6 rounded-2xl border bg-white p-5 dark:bg-card">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Versões do relatório</h2><p className="mt-1 text-sm text-muted-foreground">Cada arquivo permanece armazenado de forma privada para conferência e auditoria.</p></div>{council.status !== "completed" && council.current_import_id && <Button variant="outline" size="sm" asChild><Link href={`/hub/conselhos/${councilId}/importar`}><RefreshCw className="h-4 w-4" />Importar nova versão</Link></Button>}</div>
      <div className="mt-4 divide-y rounded-xl border">
        {data.imports.map((item) => {
          const isCurrent = item.id === council.current_import_id;
          const isLatest = item.id === latestImportId;
          const canDelete = data.imports.length > 1 && !isCurrent && !isLatest;

          return <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 p-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <strong className="text-sm">Versão {item.version}</strong>
                {isCurrent && <Badge variant="success">Atual</Badge>}
                {isLatest && !isCurrent && <Badge variant="secondary">Mais recente</Badge>}
                <Badge variant="outline">{importStatusLabels[item.status] ?? item.status}</Badge>
              </div>
              <p className="mt-1 truncate text-sm text-muted-foreground">{item.original_file_name} · {(item.file_size_bytes / 1024 / 1024).toLocaleString("pt-BR", { maximumFractionDigits: 1 })} MiB · enviada em {new Date(item.created_at).toLocaleString("pt-BR")}</p>
            </div>
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" asChild><a href={`/api/class-councils/${councilId}/imports/${item.id}/file`}><Download className="h-4 w-4" />Baixar</a></Button>
              {canDelete && <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="ghost" size="sm" disabled={deletingImportId !== null} className="text-destructive hover:bg-destructive/10 hover:text-destructive"><Trash2 className="h-4 w-4" />Excluir</Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Excluir a versão {item.version}?</AlertDialogTitle>
                    <AlertDialogDescription>O arquivo e os dados importados desta versão histórica serão excluídos definitivamente. A versão atual do conselho permanecerá intacta.</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel disabled={deletingImportId === item.id}>Cancelar</AlertDialogCancel>
                    <AlertDialogAction disabled={deletingImportId !== null} onClick={() => void deleteImport(item.id)} className="bg-destructive text-white hover:bg-destructive/90">
                      {deletingImportId === item.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                      {deletingImportId === item.id ? "Excluindo" : "Excluir versão"}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>}
    ...
    <Dialog open={activeMetric !== null} onOpenChange={(open) => { if (!open) { setActiveMetric(null); setDetailSearch(""); } }}>
      <DialogContent className="max-h-[85vh] grid-rows-[auto_auto_minmax(0,1fr)] sm:max-w-2xl">
        ...
        <div className="min-h-0 space-y-2 overflow-y-auto pr-1">
          {activeMetric === "pendingInterventions" ? detailedInterventions.map((item) => <div key={item.id} className="rounded-xl border bg-muted/20 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium">{item.description}</p><p className="mt-1 text-sm text-muted-foreground">{item.studentName ? `${item.studentName} · ` : ""}{item.className}</p></div><Badge variant="outline">{interventionStatusLabels[item.status] ?? item.status}</Badge></div>{(item.responsible_name || item.due_date) && <p className="mt-2 text-sm text-muted-foreground">{item.responsible_name ? `Responsável: ${item.responsible_name}` : "Sem responsável"}{item.due_date ? ` · Prazo: ${new Date(`${item.due_date}T12:00:00`).toLocaleDateString("pt-BR")}` : ""}</p>}</div>) : detailedStudents.map((student) => <div key={student.enrollmentId} className="rounded-xl border bg-muted/20 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-medium">{student.name}</p><p className="mt-0.5 text-sm text-muted-foreground">Matrícula {student.enrollmentNumber}</p></div><Badge variant="outline">Turma {student.className}</Badge></div><p className="mt-2 text-sm text-muted-foreground">{activeMetric === "lowAttendance" ? `Frequência anual: ${formatAttendance(student.attendanceRate)}` : activeMetric === "worsened" ? `${student.alerts.previousLowGradeCount ?? 0} → ${student.alerts.currentLowGradeCount} disciplinas com nota abaixo de 6,0` : activeMetric === "atRisk" ? student.alerts.reasons.join(" · ") : `Frequência anual: ${formatAttendance(student.attendanceRate)}`}</p></div>)}
...
function OccurrenceSummary({ summary }: { summary: StudentOccurrenceSummary }) {
  ...
  return <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50/70 px-2.5 py-2 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/25 dark:text-amber-200"><strong>{summary.count} {summary.count === 1 ? "ocorrência" : "ocorrências"}</strong><span className="mx-1.5">·</span>Última: {STUDENT_OCCURRENCE_LABELS[summary.latest.category]}</div>;
}
