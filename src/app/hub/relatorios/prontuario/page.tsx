...
function OccurrencesSection({ data }: { data: StudentProfileData }) {
  return <section className="mt-6"><SectionTitle>Ocorrências</SectionTitle>{data.occurrences.length ? <div className="mt-2 divide-y rounded-lg border border-slate-300">{data.occurrences.map((item) => <article key={item.id} className="record-avoid-break p-2.5"><div className="flex items-start justify-between gap-3"><strong className="text-[10px]">{STUDENT_OCCURRENCE_LABELS[item.category]}{item.targetType === "collective" ? ` · Coletiva com ${item.participants.length} estudantes` : ""}</strong><span className="text-[8px] text-slate-500">{formatDate(item.occurredOn)} · {item.guardianNotified ? "responsável ciente" : "responsável não cientificado"}</span></div>{item.targetType === "collective" ? <p className="mt-1 text-[8px] text-slate-500">Envolvidos: {item.participants.map((participant) => participant.name).join(", ")}</p> : null}{item.notes ? <p className="mt-1 text-[9px] leading-4">{item.notes}</p> : null}</article>)}</div> : <p className="mt-3 text-[10px] italic text-slate-500">Nenhuma ocorrência registrada.</p>}</section>;
}
