...
export function ClassOccurrencesPanel({ councilId, classId, active }: { councilId: string; classId: string; active: boolean }) {
  ...
  return <section className="rounded-2xl border bg-white p-5 dark:bg-card sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-700 dark:text-amber-300">Ano letivo de {data.schoolYear}</p><h2 className="mt-2 text-xl font-black">Ocorrências da turma e dos estudantes</h2><p className="mt-1 text-sm text-muted-foreground">Todos os registros do ano, independentemente do bimestre em que foram cadastrados.</p></div><div className="flex items-center gap-3"><strong className="text-2xl">{data.total}</strong><Button variant="outline" asChild><Link href="/hub/ocorrencias">Abrir módulo</Link></Button></div></div>
...
function OccurrenceGroup({ icon: Icon, title, items, empty }: { icon: typeof UsersRound; title: string; items: StudentOccurrence[]; empty: string }) {
  return <section><div className="mb-3 flex items-center gap-2"><Icon className="size-4 text-violet-700" /><h3 className="font-bold">{title}</h3><span className="text-xs text-muted-foreground">({items.length})</span></div>{items.length ? <div className="divide-y rounded-xl border">{items.map((item) => <OccurrenceLine key={item.id} item={item} />)}</div> : <p className="rounded-xl border border-dashed p-5 text-sm text-muted-foreground">{empty}</p>}</section>;
}

function OccurrenceLine({ item }: { item: StudentOccurrence }) {
  return <article className="p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{STUDENT_OCCURRENCE_LABELS[item.category]}</strong>{item.targetType === "collective" ? <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold uppercase text-amber-800 dark:bg-amber-950/60 dark:text-amber-200">Coletiva · {item.participants.length} estudantes</span> : null}</div><span className="text-xs text-muted-foreground">{formatDate(item.occurredOn)}</span></div>{item.notes ? <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700 dark:text-slate-300">{item.notes}</p> : null}<p className="mt-2 text-[11px] text-muted-foreground">{item.guardianNotified ? "Responsável(is) ciente(s)" : "Responsável(is) não cientificado(s)"}{item.createdByName ? ` · Registrado por ${item.createdByName}` : ""}</p></article>;
}
