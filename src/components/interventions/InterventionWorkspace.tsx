...
export function InterventionWorkspace({ readOnly = false, initialFilters, backHref = "/hub" }: { readOnly?: boolean; initialFilters?: InterventionWorkspaceFilters; backHref?: string }) {
  ...
  return <main className={`intervention-report-root mx-auto w-full max-w-7xl p-4 py-8 sm:p-6 lg:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px] ${mode === "compact" ? "report-compact" : "report-follow-up"}`}>
    ...
    <article className={readOnly ? "intervention-print-document mt-6 bg-white text-slate-950 shadow-sm print:mt-0 print:shadow-none" : "intervention-operational-list mt-6"}>
      ...
      {readOnly ? <footer className="mt-8 border-t border-slate-300 pt-2 text-[9px] text-slate-500">Documento operacional gerado pelo FélixHub. A origem identifica onde a ação foi definida; o status reflete o acompanhamento atual.</footer> : null}
    </article>
      ...
      <form onSubmit={submit} className="flex max-h-[90vh] min-h-0 flex-col">
        <DialogHeader className="shrink-0 border-b px-6 pb-4 pt-5 pr-14 text-left">
          <div className="flex items-start gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-sky-100 text-sky-700 dark:bg-sky-950/60 dark:text-sky-300"><ClipboardCheck className="size-5" /></span>
            <div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2">
              ...
              {selectedStudent ? <div className="flex items-center justify-between gap-3 rounded-xl border border-sky-200 bg-sky-50/80 px-3.5 py-3 dark:border-sky-900 dark:bg-sky-950/30">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="grid size-9 shrink-0 place-items-center rounded-full bg-white text-sky-700 shadow-sm dark:bg-slate-900 dark:text-sky-300"><UserRound className="size-4" /></span>
                ...
                <div className="relative">
                  ...
                  {studentsQuery.isPending ? <Loader2 aria-label="Carregando estudantes" className="absolute right-3.5 top-1/2 size-4 -translate-y-1/2 animate-spin text-sky-600" /> : null}
                </div>
                <div aria-live="polite">
                  ...
                  {!studentsQuery.isPending && normalizedSearch.length >= 2 ? <div className="max-h-44 overflow-y-auto rounded-xl border bg-background shadow-sm">
                    {results.length ? results.map((student) => <button key={student.studentId} type="button" onClick={() => { setStudentId(student.studentId); setStudentSearch(""); }} className="flex w-full items-center justify-between gap-3 border-b px-3.5 py-2.5 text-left transition last:border-0 hover:bg-sky-50 focus-visible:bg-sky-50 focus-visible:outline-none dark:hover:bg-sky-950/30 dark:focus-visible:bg-sky-950/30">
